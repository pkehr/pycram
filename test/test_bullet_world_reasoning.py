...
class TestCaseBulletWorldReasoning(BulletWorldTestCase):
    ...
    def test_visible(self):
        ...
        time.sleep(1)
        camera_frame = get_robot_description().get_camera_frame()
        ...
        self.assertTrue(btr.visible(self.milk, self.robot.get_link_pose(camera_frame),
                                    get_robot_description().front_facing_axis))

    def test_occluding(self):
        self.milk.set_pose(Pose([3, 0, 1.2]))
        self.robot.set_pose(Pose())
        self.assertTrue(btr.occluding(self.milk, self.robot.get_link_pose(get_robot_description().get_camera_frame()),
                                      get_robot_description().front_facing_axis) != [])

    def test_reachable(self):
        self.robot.set_pose(Pose())
        time.sleep(1)
        self.assertTrue(btr.reachable(Pose([0.5, -0.7, 1]), self.robot, get_robot_description().get_tool_frame("right")))
        self.assertFalse(btr.reachable(Pose([2, 2, 1]), self.robot, get_robot_description().get_tool_frame("right")))

    def test_blocking(self):
        self.milk.set_pose(Pose([0.5, -0.7, 1]))
        self.robot.set_pose(Pose())
        time.sleep(2)
        self.assertTrue(btr.blocking(Pose([0.5, -0.7, 1]), self.robot, get_robot_description().get_tool_frame("right")) != [])
