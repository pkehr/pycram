...
from demos.pycram_multirobot_thesis_demos.hsrb_turtle_experiment.methods.actions import navigate_to_many_points, \
    transport_object
from demos.pycram_multirobot_thesis_demos.hsrb_turtle_experiment.methods.nav_poses import NavOptions, NavPoses
...
from demos.pycram_multirobot_thesis_demos.simulation_demos.demo_manager import robot_two
...
def start_hsrb_navigation(demo_scenario: ScenarioSelection, starting_nav_pose, table_nav_pose, robot_mode, robot):
    with robot_mode:
        if demo_scenario.navigate_start_hsrb:
            NavigateAction(target_locations=[starting_nav_pose], used_robot=robot).resolve().perform()
            rospy.loginfo(f"{robot.name} is at navigation position")

        if demo_scenario.navigate_table_one_hsrb:
            NavigateAction(target_locations=table_nav_pose, used_robot=robot).resolve().perform()
            rospy.loginfo(f"{robot.name} is at the first table")


def start_turtle_navigation(demo_scenario: ScenarioSelection, starting_nav_pose, robot_mode, robot):
    with robot_mode:
        if demo_scenario.navigate_start_turtle:
            starting_pose_turtle_rotated = Pose(position=starting_nav_pose.position,
                                                orientation=rotated_quaternion((-90)))

            NavigateAction(target_locations=[starting_nav_pose], used_robot=robot).resolve().perform()
            NavigateAction(target_locations=[starting_pose_turtle_rotated], used_robot=robot).resolve().perform()
            rospy.loginfo(f"{robot.name} is at starting position")


def table_turtle_navigation(demo_scenario: ScenarioSelection, nav_poses, robot_mode, robot):
    with robot_mode:
